...
;(function($, window) {
	...
	jcf.addModule({
		...
		options: {
			...
			buttonText: '',
			placeholderText: 'Click here to browse files',
...
$(document).ready(function(){
	(function($){
		$(window).load(function(){
			...
			var amount=Math.max.apply(Math,$(".gallery .pagination li").map(function(){return $(this).outerWidth(true);}).get());
				
			$(".gallery .pagination").mCustomScrollbar({
				scrollInertia:500,
				axis:"x",
				theme:"light-3",
				advanced:{
			...
			if($(window).width() >= 768){
				var prodAreaHeight = $('.proddetail-area').innerHeight();
				var paginationHeight = $('.gallery').find('ul').height();
				var calcHeight = prodAreaHeight - paginationHeight -20;
				$('.gallery .mask').height(calcHeight);
			}
		});
	})(jQuery);
	
	var container = document.querySelector('.testi-list');
	...
	$('.tabset').on('click', 'a', function(e){
		...
		if(current.hasClass('active')){
			...
			$('.tabset').find('p').animate({'opacity':'0'}, function(){
				$('.tabset').find('.text-area').width('0').height('0');
				current.find('.text-area').width('48%').height('auto');
			});
			$('.tabset').find('li').removeClass('active');
			current.addClass('active');
			setTimeout(function(){
				$('.tabset').find('.active p').animate({'opacity':'1'});
			}, 800)
		}
	});
});