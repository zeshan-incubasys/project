<?php  include('include/header.php');?>
	<section class="banner candidate">
		<div class="container">
			<h1>Candidates – Work &amp; Live Your Dream!</h1>
			<span class="title">For Jobseekers &amp; candidates looking for that perfect opportunity</span>
		</div>
	</section>
	<div class="crew container">
		<div class="text-area col-md-5">
			<img src="images/candidates.png" alt="" class="img">
			<p>Whether you are an experienced candidate or a new talent, it is our goal to help advance your career and hopefully contribute to securing your next job. We are committed to assist and advice. You will be supported by our hands-on approach, an honest assessment matching realistic opportunities. </p>
			<p>Do you have what it takes to find the job of your dreams………?</p>
			<p>The yachting industry is expanding, certified and professional crewmembers are in demand.</p>
		</div>
		<div class="col-md-5 col-md-offset-2 listing">
			<div class="holder">
				<a href="cv.php" class="butn">Submit CV</a>
				<h3>Who We Serve</h3>
				<ul>
					<li>Certified &amp; Professional crew looking for a perfect opportunity</li>
					<li>Fresh talent looking for a start in the industry</li>
				</ul>
				<h3>Who &amp; What we look for </h3>
				<ul>
					<li>Service-minded &amp; Safety Conscious </li>
					<li>Hard Workers but Presentable</li>
					<li>An Eye for Attention to Details</li>
				</ul>
				<p>Those are just a few of the keywords on the list of qualifications, traits a Client will be looking for in a potential candidate.</p>
			</div>
		</div>
	</div>
	<div class="image-section container crew">
		<div class="holder">
			<div class="col-md-5 image visible-lg visible-md">
				<img src="images/candidate1.jpg" alt="">
			</div>
			<div class="col-md-7 image img2">
				<img src="images/candidate2.jpg" alt="">
				<a href="cv.php" class="butn cv">Submit CV</a>
			</div>
		</div>
		<p>To work on board a luxury yacht and be part of a unique lifestyle, you must remember standards and expectations are high. You have to be good - very good - but you also have to fit in, be a team-player, usually working long hours and living in tight quarters with shared crew cabins. Being employed by some of the World’s most affluent personalities, requires utmost discretion.</p>
		<p>Backed by references proving that they can deliver consistent and exceptional service, the successful candidate, should have the passion, flexibility and the right attitude to fulfill his/her ambitions. </p>
		<p>Are you determined to join, do you want to pursue a career in yachting?</p>
		<p>Send your CV and Register with MARINA RECREWTER</p>
		<h3>GENERAL INFORMATION</h3>
		<p>The purpose of this section is mainly to try to assist prospective new talents who have little or no previous seagoing experience and to provide more experienced candidates with basic information and requirements. MARINA RECREWTER is committed to support your career, ready to coach and advise. </p>
		<p>A large yacht is operated either as a private or commercial (charter) vessel. An increasing number of yachts are exceeding 3000 GT, which require full commercial merchant certification for all deck and engineering officers. Most yachts are ISM/ISPS MLC2006 compliant through classification and flag state.</p>
		<p>Years of experience and qualifications related to a certain position is of course essential, but there are a few tips to add to your CV: Dive certificate, first aid/medical course, hospitality service, languages, mechanic/carpentry, water sports experience, computer skills, driving license, boat handling, nursing etc In fact, any extra skill/hobby, you might see as a potential advantage to have on board. Longevity in previous held positions is a must for obvious reasons, as are great references and positive feed-back.</p>
		<p>Before being considered, all crewmembers should as a minimum hold; a STCW95 and ENG1 certificates. Additional courses, such as crowd management, fast rescue boat, advanced firefighting, helicopter ops. Security awareness, will depend on and correspond with your safety responsibilities on board the yacht’s emergency muster plan. </p>
		<p>STCW95: As a requirement to comply with the International Maritime Organization (IMO) Standards of Training, Certification and Watchkeeping (STCW95) Basic Safety Training (BST); basic firefighting, personal sea survival, first aid and social responsibility. Usually, a 5 days course, but make sure the training facility is IMO recognized. The course is available in maritime centers around the world or in your home country. In the UAE contact EIMA Dubai Maritime city or ESTDC in Abu Dhabi. </p>
		<p>ENG1: Medical fitness certificate for seafarers, conducted by appointed doctors approved by the governing maritime administration.</p>
		<p>If you are a new talent, with no previous maritime experience, we encourage you to assist on smaller boats with day work, if for nothing else than to display determination on your CV and give you an initial edge. This is also a useful way to gain experience and assure yourself of a career in yachting. Mandatory safety training, added a relevant course, day-work or dock-walking, can eventually lead to a junior position. Positive and enthusiastic personality; can-do attitude, flexibility, willingness to learn, team-player, should all be integrated adjectives of your profile.</p>
		<span class="note">For any requests or further information, please contact us.</span>
	</div>

	  <section class="what-do">
        <div class="container head">
			  <a href="#myModal3" role="button" data-toggle="modal"><h2><span>WHY MARINA RECREWTER? </span></h2></a>
          <p>We have experienced firsthand the effects of not having the right profile on board; inadequate manning requirements, high turnover – increased costs and most importantly the dissatisfaction of Owners and Guests. Getting the right candidate first time is our priority. </p>
        </div>
	</section>

<?php  include('popup.php');?>
<?php include('include/footer.php');?>