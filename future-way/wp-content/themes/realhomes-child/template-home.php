...
?>
	  ...
	  <div>
			<div class="sub-head" style="text-align: left">
			...
			<div class="property-items-container clearfix">
				  ...
				  if (is_front_page()) {
						...
						/*$paged = (get_query_var('page')&&get_query_var('type')=='sale') ? get_query_var('page') : 1;
						$type = (get_query_var('type')) ? get_query_var('type') : "sale";*/
				  }

				  /*if($_REQUEST){
						print_r($_REQUEST);
						print_r(get_query_var('type'));die;
				  ...
				  if ($home_properties_query->have_posts()) :
						$post_count = 0;
						//print_r($home_properties_query);die('===');
			...
			<?php //theme_pagination_home($home_properties_query->max_num_pages,'sale'); ?>
	  </div>
	  <div>
			<div class="sub-head" style="text-align: left">
			...
			<div class="property-items-container clearfix">
				  ...
				  if (is_front_page()) {
						$paged = (get_query_var('page')) ? get_query_var('page') : 1;
						/*$paged = (get_query_var('page')&& get_query_var('type')=='rent') ? get_query_var('page') : 1;
						$type = (get_query_var('type')) ? get_query_var('type') : "rent";*/
				  }
			...
			<?php theme_pagination($home_properties_query->max_num_pages); ?>
			<?php //theme_pagination_home($home_properties_query->max_num_pages,'rent'); ?>
	  </div>
